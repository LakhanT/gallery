...
export function createStore(env, origin) {
  ...
  return {
    ...
    async listPhotosNeedingReindex(
      limit = 10,
      { includeFailed = false, includeQueued = false } = {}
    ) {
      const gallery = await this.getGallery();
      const versions = await this.getFaceVersions();
      const statusMap = await this.getFaceIndexStatusMap();
      const max = Math.min(Math.max(1, Number(limit) || 10), gallery.photos.length || 1);
      return gallery.photos
        .filter((photo) => {
          ...
          if (!includeQueued && (st?.status === "queued" || st?.status === "processing")) {
            return false;
          }
          return true;
        })
