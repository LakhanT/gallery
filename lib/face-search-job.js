 ...
 *
 * Dispatch:
 *   PRODUCTION/STAGING — Cloudflare Queue (FACE_SEARCH_QUEUE) + durable consumer Worker
 *   LOCAL              — in-memory gatewayPending / setImmediate / waitUntil fallback
 */
...
export const JOB_EXPIRED = "expired";
...
export function publicJobView(job, env = {}) {
  ...
  if (job.status === JOB_FAILED || job.status === JOB_EXPIRED) {
    return {
      ...base,
      error: job.error || (job.status === JOB_EXPIRED ? "Search expired." : "Face search failed."),
      busy: false,
    };
  }
...
export async function acceptFaceSearchJob({
  ...
  const objectKey = tempSelfieObjectKey(id);

  if (!skipProcess && !synthetic) {
    await store.putFaceSearchTempImage(id, bytes, contentType || "image/jpeg");
    if (typeof store.setFaceSearchJobR2Key === "function") {
      await store.setFaceSearchJobR2Key(id, objectKey);
    }
  }
  ...
  } else if (!synthetic && hasDurableSearchQueue(env)) {
    // PRODUCTION / STAGING: durable Cloudflare Queue (consumer Worker drains)
    await enqueueFaceSearchMessage(env, {
      jobId: id,
      objectKey,
      attempt: 1,
    });
  ...
  return {
    ...
    httpStatus: 202,
    body: {
      ...publicJobView(job, env),
      dispatch: hasDurableSearchQueue(env) && !skipProcess && !synthetic ? "cloudflare-queue" : "local",
    },
  };
}
 ...
 * @param {{ alreadyClaimed?: boolean, preferQueueErrors?: boolean }} [options]
 */
export async function processFaceSearchJob(
  store,
  env,
  jobId,
  filename = "selfie.jpg",
  ...
  const job = await store.getFaceSearchJob(jobId);
  if (!job) return;
  if (job.status === JOB_COMPLETED || job.status === JOB_FAILED || job.status === JOB_EXPIRED) {
    return;
  }
  if (job.expiresAt && Date.parse(job.expiresAt) < Date.now()) {
    await store.updateFaceSearchJob(jobId, {
    ...
    try {
      ...
      if (error.status === 429 || error.status === 503) {
        ...
        if (retries < 8) {
          ...
          return processFaceSearchJob(store, env, jobId, filename, options);
        }
